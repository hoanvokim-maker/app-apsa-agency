...
if ($t !== '' && preg_match('/^[A-Za-z0-9]{8,64}$/', $t)) {
    try {
        $st = apsa_og_pdo()->prepare('SELECT `title`, `note`, `active` FROM `video_playlists` WHERE `token` = ? LIMIT 1');
        $st->execute(array($t));
        $r = $st->fetch();
        if ($r && (int) $r['active']) {
...
apsa_og_wrap(__DIR__ . '/playlist.html', '/playlist.html', $title, $desc, 'video.other');
